...
+ (void)load
{
    hookMethod(objc_getClass("NSObject"), @selector(methodSignatureForSelector:), [self class], @selector(hook_methodSignatureForSEL:));
    hookMethod(objc_getClass("NSObject"), @selector(forwardInvocation:), [self class], @selector(hook_forwardInvocation:));
}

- (NSMethodSignature *)hook_methodSignatureForSEL:(SEL)arg1
{
    if (![self respondsToSelector:arg1]) {
        _errorFunctionName = NSStringFromSelector(arg1);
        NSMethodSignature *methodSignature = [self hook_methodSignatureForSEL:arg1];
        if (class_addMethod([self class], arg1, (IMP)dynamicMethodIMP, "v@:")) {
            NSLog(@"临时方法添加成功！");
        }
        if (!methodSignature) {
            methodSignature = [self hook_methodSignatureForSEL:arg1];
        }
        return methodSignature;
    }else{
        return [self hook_methodSignatureForSEL:arg1];
    }
}

- (void)hook_forwardInvocation:(NSInvocation *)arg1
{
    SEL sel = [arg1 selector];
    if ([self respondsToSelector:sel]) {
        [arg1 invokeWithTarget:self];
    } else {
        [self hook_forwardInvocation:arg1];
    }
}
@end
